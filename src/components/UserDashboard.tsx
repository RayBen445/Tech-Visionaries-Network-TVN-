...
import { Save, Loader2, Upload, User, AlertCircle, CheckCircle, LogOut, ArrowLeft } from 'lucide-react';
...
export default function UserDashboard() {
  ...
  const fetchProfile = async (userId: string) => {
    try {
      ...
      if (data) {
        ...
        // Handle case where trigger didn't fire or row doesn't exist yet
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const optimized = await optimizeImage(file, 400); // 400px width max for avatars
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: err.message || 'Failed to save profile. Username might be taken or permissions denied.' });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage your builder profile and identity.</p>
          </div>
